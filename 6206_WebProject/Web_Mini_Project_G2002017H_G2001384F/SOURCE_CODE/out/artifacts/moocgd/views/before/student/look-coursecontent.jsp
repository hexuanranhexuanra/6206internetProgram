...
<body>
	...
	<div class="wrap">
		<div class="coursecontent-head">
			<ul>
				...
				<li><form action="/moocgd/before/student/listask" method="post" style="display:inline-block;">
						<button class="taskbtn">Assignments</button>
					</form></li>
				<li><a href="/moocgd/before/file/tostudentfilemaven" style="color:black;">Files</a></li>
				<li onclick="showdiscuss();">Discussion Board</li>
			</ul>
		</div>
		<div class="catalog">
			<div class="tittle">Catalogue</div>
			<!-- 学生选课系统的最后的一部分;coursemsg去了lookframeset.jsp页面 -->
			<form action="../views/before/student/look-frameset.jsp" method="post" style="display:inline-block;">
				<!-- 课程模块，重要的是课程id -->
				<input type="hidden" id="courseid" name="courseid" value="${course.id}">
				<button class="editorcontent">View the course content</button>
			</form>
	...
	<script type="text/javascript">
	     ...
	     function showdiscuss(){
	        ...
	        $.post(
	          "/moocgd/before/discussblock/themes",
	          function(data){
	              if(data==''){
	                 notfounddiscuss();
	              }else{
	    ...
	    function discuss(){
	      ...
	      '<input id="issue" style="width:500px;height:30px;" placeholder="New topic" class="layui-input"/>'+
	      '<span onclick="layuiissue();" class="layui-btn" style="height:30px;position:absolute;margin-left:505px;margin-top:-30px;line-height:30px;">Publish</span>'+
	      '</legend>'+
	    ...
	    function layuiissue(){
	        layui.use(['layer'], function(){
	            ...
	            if($("#issue").val()!=null && $("#issue").val().trim()!=''){
	                 $.post(
	                     "/moocgd/before/discussblock/issue",
	                     {"theme":$("#issue").val()},
	                     function(data){
	                         if(data!='null'){
	                             layer.msg('Publish successed');
	                             showdiscuss();
	                         }else{
	                             layer.msg('Publish failed');
	                         }
	                     }
	                 );
	             }else{
	                 layer.msg('Please enter the post');
	             }
	        });
	    }
	    /*没有话题则提示发布话题*/
	    function notfounddiscuss(){
	      ...
	      '<span style="position:absolute;margin-left:-240px;margin-top:270px;font-size:18px;color:#999;line-height:2;">There is no topic yet, go and publish one!</span>'+
	      '</div>');
	    }
			     ...
			     //调用话题分页
				   laypage.render({
						    ...
						    /*一页能显示完内容，则不再显示分页*/
						      ...
						      document.getElementById('discussfield').innerHTML=function(){
						          ...
						          layui.each(thisData, function(index,item){
						              ...
						              var PostingHTML;
						              if(user==Posting){
						                  PostingHTML = '<div class="operation" id="layerDemo" style="color:#7a9e31;position:absolute;margin-left:850px;margin-top:-40px;"><span onclick="deletetheme('+item.id+');" style="cursor:pointer">删除</span>&nbsp|&nbsp<span onclick="edittheme('+item.id+');" class="edittheme" data-method="notice" style="cursor:pointer">Edit</span></div>';
						              }else{
						                  PostingHTML='';
						              }
						              if(item.state==1){
						              ...
						              arr.push('<div class="existdiscuss" style="background-color:#fcfcfc;position:relative;margin-bottom:5px;margin-left:180px;width:963px;height:157px;border:1px solid #e6e6e6;">'+
								              ...
								              '</div>'+
								              PostingHTML+
								               '<div style="width:200px;margin-left:90px;margin-top:-42px;"><span style="font-size:14px;color:#7a9e31">'+item.username+'</span><span style="margin-left:10px;color:#666">NTU</span></div>'+
								                ...
								                '<span class="layui-btn" onclick="ensurereply('+item.id+','+index+');" style="background-color:#7b9e31;position:absolute;margin-left:505px;margin-top:-30px;line-height:30px;height:30px;">Reply</span>'+
								                '</div>'+
								          '</div>');
						          });
						           return arr.join('');
					...
					window.deletetheme=function(themeid){
					        layer.confirm("Are you sure you want to delete it? It cannot be restored after deletion.", { title: "Delete confirmation",offset:"200px" }, function (index) {  
			                    layer.close(index);  
			                    $.post(
			                        "/moocgd/before/discussblock/deletetheme",
			                        {"id":themeid},
			                        function(data){
			                            if(data!='null'){
					...
					window.edittheme=function(themeid){
					      ...
					      layer.open({
					        ...
					        ,content: '<div style="padding: 50px;background-color: #393D49; font-weight: 300;"><textarea class="edittextarea" style="height:125px;width:201px;line-height:22px;" class="layui-input" placeholder="Please enter what you want to change"></textarea></div>'
					         ,yes:function(index, layero){
					             if($(".edittextarea").val()!=null&&$(".edittextarea").val().trim()!=''){
					                 $.post(
					                     "/moocgd/before/discussblock/updatetheme",
					                     {"id":themeid,"theme":$(".edittextarea").val()},
					                     function(event){
					                         if(event!='null'){
					                             layer.msg('Modify successfully',{offset:'300px'});
					                             showdiscuss();
					                         }else{
					                             layer.msg('Fail to modify',{offset:'300px'});
					                         }
					                     }
					                 );
					                 layer.closeAll();
					             }else{
					                 layer.msg('Please enter what you want to change',{offset:'300px'});
					             }
					         }
					      });
					}
                   ...
                   var talkID;//话题id
			          ...
			          ind = index+limit*(curr-1);//?????????????????????????????????????????????????????????
			           ...
			           '<div class="existdiscuss" style="background-color:#fcfcfc;margin-top:30px;position:relative;margin-left:180px;width:963px;height:157px;border:1px solid #e6e6e6;">'+
						'<div><img style="margin-left:30px;margin-top:20px;width:42px;height:42px;border-radius:100%;" src="img/coursetest.jpg">'+
						    imagetheme+
						'</div>'+
						'<div style="width:200px;margin-left:90px;margin-top:-42px;"><span style="font-size:14px;color:#7a9e31">'+data[ind].username+'</span><span style="margin-left:10px;color:#666">NTU</span></div>'+
						...
						'</i>'+
						'<div class="replyevent" style="width:500px;margin-left:89px;margin-top:50px;display:none;"><input class="layui-input replyContent" style="width:500px;height:30px;"/>'+
						'<span class="layui-btn" style="background-color:#7b9e31;position:absolute;margin-left:505px;margin-top:-30px;line-height:30px;height:30px;">Reply</span>'+
						'</div>'+
						'</div>'+
	                     ...
	                     '<textarea class="layui-textarea" placeholder="请输入内容..." style="width:963px;"></textarea>'+
	                        '<span class="layui-btn ensurereply" style="background-color:#7b9e31;position:absolute;margin-left:505px;margin-top:-30px;line-height:30px;height:30px;">Reply</span>'+
	                     '</div>'+
			           '</div>');
			           if(data[ind].replys==''){
			               notFoundReply();
			           ...
			           $(".ensurereply").on('click',function(){
			               if($(".layui-textarea").val()!=null && $(".layui-textarea").val().trim()!=''){
			                   ...
			                   $.post(
							           "/moocgd/before/discussblock/reply",
							           {
							           "tid":data[ind].id,
							           "uid":"${USER.id}",
							           ...
							           function(data){
							               if(data!='null'){
							                   layer.msg('回复成功');
							                   $.post(
							                      "/moocgd/before/discussblock/queryreply",
							                       {"ind":talkID},
							                       function(event){
							                           FoundReply(event);
							                       }
					...
					window.deletereply=function(replyid){
					    ...
					    layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认",offset:"200px" }, function (index) {  
			                    layer.close(index);  
			                     $.post(
			                        "/moocgd/before/discussblock/deletereply",
			                        {"id":replyid},
			                        function(data){
			                            if(data!='null'){
			                                layer.msg('删除成功');
					      ...
					      layer.open({
					        ...
					        ,content: '<div style="padding: 50px;background-color: #393D49; font-weight: 300;"><textarea class="edittextarea" style="height:125px;width:201px;line-height:22px;" class="layui-input" placeholder="请输入要修改的内容"></textarea></div>'
					         ,yes:function(index, layero){
					             if($(".edittextarea").val()!=null&&$(".edittextarea").val().trim()!=''){
					                 $.post(
					                     "/moocgd/before/discussblock/updatereply",
	   ...
	   function ensurereply(tid,index){
		...
		$(".addparent").click(function(){
			$.post("/moocgd/before/course/insertFirstCatalog",{
			"courseid":$("#courseid").val()
			},function(data){
				window.location.href="/moocgd/before/course/coursecontent";
			});
		});
		function deleteBtn(){
